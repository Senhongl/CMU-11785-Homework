...
def training(opt, encoder, decoder, train_loader, val_loader):
    ...
    for epoch in range(opt.n_epoch):
        ...
        for batch_idx, (utterances, labels, u_lens, l_lens) in enumerate(train_loader):
            ...
            _, _, out_lens, hidden = encoder(utterances, u_lens)

            hidden = (hidden[0][-2:].permute(1, 0, 2), hidden[1][-2:].permute(1, 0, 2))
            hidden = (hidden[0].reshape(hidden[0].size(0), -1), hidden[1].reshape(hidden[1].size(0), -1))

            predict_labels = decoder(text = labels, hidden = hidden, mode = 'pre_training')
...
if __name__ == '__main__':
    ...
    opt.model_name = 'pre_train_model'
    options.printer(opt)
